...
for (let index = 0; index < 6; index++) {
    ...
    client.on('voiceStateUpdate', async (prev, cur) => {
        ...
        if (cur.channel && (cur.channel.id === chnls[index])) {
            ...
            if (selamlı.includes(cur.member.id) && (cur.member.roles.highest.rawPosition < cur.guild.roles.cache.get("784479711295176734").rawPosition)) { //member rol id
                ses = await concon.play('./tekrardan.mp3');
                return;
            }
            if ((cur.member.roles.highest.rawPosition < cur.guild.roles.cache.get("784479711295176734").rawPosition)) { //member ROLID
                ses = await concon.play('./hosgeldin.mp3');
                selamlı.push(cur.member.user.id);
            } else if (cur.member.roles.highest.rawPosition > cur.guild.roles.cache.get('789626676870512711').rawPosition) {//teyit sorumlısuROLID
